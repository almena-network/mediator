...
- `crates/node/` — `almena-node`: the service.
  - `config` reads `ALMENA_*` env vars; `identity` holds the node's `did:web`, keys (`ALMENA_KEYS_PATH`) and DID document.
  - `store/`: the `Store` trait with Redis and in-memory implementations, and a contract test both must pass.
  - `mediator/`: unpacks what reaches `/didcomm` or `/ws` and dispatches — `protocols.rs` (Trust Ping, Discover Features, problem reports), `mediation.rs` (Coordinate Mediation, `forward`), `pickup.rs` (Message Pickup), `live.rs` (live-delivery sessions), `relay.rs` (forwarding to other nodes).
  ...
  - `examples/smoke.rs`: end-to-end client against a running node (`task smoke`).
- `Dockerfile`, `compose.yml` (node + Redis), `.env.example` — container build and local run. `data/` (local keys) is git-ignored.
...
- `almena-didcomm` denies `unwrap`/`expect` outside tests (`clippy.toml` allows them in tests) and uses `thiserror`; the node binary uses `anyhow`.
- Every HTTP endpoint is declared with `#[utoipa::path]` and registered through `OpenApiRouter` in `crates/node/src/routes.rs`, so it appears in `/openapi.json` and `/docs`. Keep the README endpoint table in sync.
